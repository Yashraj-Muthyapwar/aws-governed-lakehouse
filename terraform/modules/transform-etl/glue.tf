resource "aws_glue_job" "ratings_transformation_job" {
  ...
  command {
    name            = "glueetl"
    script_location = "s3://${var.scripts_bucket_name}/json_transform.py"
    python_version  = 3
  }
...
resource "aws_glue_job" "csv_transformation_job" {
  ...
  command {
    name            = "glueetl"
    script_location = "s3://${var.scripts_bucket_name}/batch_transform.py"
    python_version  = 3
  }
...
resource "aws_glue_job" "ratings_to_iceberg_job" {
  ...
  command {
    name            = "glueetl"
    script_location = "s3://${var.scripts_bucket_name}/ratings_to_iceberg.py"
    python_version  = 3
  }
